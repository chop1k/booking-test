...
DEPLOY_TEST_DOCKERFILE_NAME    = test.Dockerfile
DEPLOY_DOCKERIGNORE_NAME       = .dockerignore
DEPLOY_NGINX_CONF_NAME         = nginx.conf
...
deploy-dev:
	...
	cp $(DEPLOY_DIR)/dev/$(DEPLOY_NGINX_CONF_NAME) $(DEPLOY_NGINX_CONF_NAME)
...
deploy-prod:
	...
	cp $(DEPLOY_DIR)/prod/$(DEPLOY_NGINX_CONF_NAME) $(DEPLOY_NGINX_CONF_NAME)
...
deploy-test:
	...
	cp $(DEPLOY_DIR)/test/$(DEPLOY_TEST_DOCKERFILE_NAME) tests/go/$(DEPLOY_TEST_DOCKERFILE_NAME)
	cp $(DEPLOY_DIR)/test/$(DEPLOY_DOCKERIGNORE_NAME) $(DEPLOY_DOCKERIGNORE_NAME)
	cp $(DEPLOY_DIR)/test/$(DEPLOY_NGINX_CONF_NAME) $(DEPLOY_NGINX_CONF_NAME)

deploy-staging:
	cp $(DEPLOY_DIR)/staging/$(DEPLOY_COMPOSE_NAME) $(DEPLOY_COMPOSE_NAME)
	cp $(DEPLOY_DIR)/staging/$(DEPLOY_APP_DOCKERFILE_NAME) $(DEPLOY_APP_DOCKERFILE_NAME)
	cp $(DEPLOY_DIR)/staging/$(DEPLOY_POLLING_DOCKERFILE_NAME) $(DEPLOY_POLLING_DOCKERFILE_NAME)
	cp $(DEPLOY_DIR)/staging/$(DEPLOY_DOCKERIGNORE_NAME) $(DEPLOY_DOCKERIGNORE_NAME)
	cp $(DEPLOY_DIR)/staging/$(DEPLOY_NGINX_CONF_NAME) $(DEPLOY_NGINX_CONF_NAME)
...
clean-test:
	rm $(DEPLOY_COMPOSE_NAME)
	rm $(DEPLOY_APP_DOCKERFILE_NAME)
	rm tests/go/$(DEPLOY_TEST_DOCKERFILE_NAME)
	rm $(DEPLOY_DOCKERIGNORE_NAME)
	rm $(DEPLOY_NGINX_CONF_NAME)
...
help:
	...
	@echo "  make deploy-staging - разложить deploy-файлы для окружения staging"
	...
	@echo "  make clean-staging  - почистить перемещённые файлы, артефакты сборки и т.д. для staging"
	@echo ""
	@echo "  make help           - показать это сообщение"
	@echo ""
	@echo "Переменные:"
	@echo "  APP_RELEASE=dev|prod|test|staging  (сейчас: $(APP_RELEASE)) - используется в run-docker и clean"